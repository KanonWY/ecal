...
using named_event_t = named_event;


/**
 * @brief create shared memory by event_name
 * @note we can use these to make two process commutincate
 */
...
/**
 *@brief destory the event_name_ corrent to the shared memory
 */
...
bool named_event_wait(named_event_t* evt_, struct timespec* ts_) {
    ...
    if (evt_->set != 0) {
        ...
        while ((ret == 0) && (evt_->set == 0)) {
            ...
            if (ts_ != nullptr) {
#    ifndef ECAL_OS_MACOS
                // if timeout, return TIMEOUT which not zero
 ...
 * @brief 带名字的事件触发
 *
 * @note 创建共享内存，用于在进程之间进行消息通知
 */
class CNamedEvent {
public:
    CNamedEvent(const CNamedEvent&)            = delete; // prevent copy-construction
    CNamedEvent& operator=(const CNamedEvent&) = delete; // prevent assignment
                                                         //

    // TODO:
    CNamedEvent(CNamedEvent&&)            = default;
    CNamedEvent& operator=(CNamedEvent&&) = default;
...
/**
 * @brief close EventHandleT
 * @TODO: USE gls to move explicit new and delete
 */
bool gCloseEvent(const EventHandleT& event_) {
    if (event_.handle == nullptr) return false;
